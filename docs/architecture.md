...
Cross Host Ops（xho）是一个基于 Rust 的远程命令执行与文件复制工具，采用 CLI + Daemon 分离架构。CLI 负责用户交互，Daemon 负责目标解析、命令审查、连接管理和命令执行。
...
**通信方式：**
- 通过 `~/.xho/xhod.sock` Unix Socket 连接本地 daemon
...
- 监听 TCP:2222 SSH Server（远程 xhod 连接）
- 目标解析（Resolver）
- 命令审查（Reviewer）
- 分发操作到 Gateway
- 多候选路由遍历（fallback）
...
```
Unix Socket (本地 CLI)  ─┐
                         ├─→ tonic gRPC Server (XhoRpcService)
SSH subsystem (远程)    ─┘        ↓
                         同一套 RPC handler 处理所有请求
```

本地连接和远程连接共用同一个 `XhoRpcService`，区别仅在入口传输层。
...
**输入：** 用户提供的 target 字符串（如 `host1`、`remote-xhod:host2`、`192.0.2.200`）
...
| **XhodGateway** | SSH subsystem → gRPC | 单个共享 `XhodClient`，远程 daemon 管并发 | gRPC ListServers RPC（返回远程 daemon 聚合的所有 Gateway 列表） |
...
| **XhodConnection** | gRPC Execute/Copy stream | XhodGateway |
...
xhod 作为 SSH 服务端接受远程连接。

**监听：** `TCP:2222`（可配置）

**支持的操作：**
- `subsystem_request("xho-rpc")` — 唯一接受的请求，将 SSH channel stream 作为 gRPC 连接处理
- `auth_publickey()` — 校验 `~/.xho/authorized_keys`
...
```
远程 xhod client (XhodGateway)
  → SSH connect to TCP:2222
  → publickey auth (user=xho)
  → request_subsystem("xho-rpc")
  → channel stream → IncomingConn::Remote
  → tonic Server 处理 (同一个 XhoRpcService)
```

## 通信协议

### gRPC 协议 (`proto/xho.proto`)
...
src/
├── bin/
│   ├── xho.rs              # CLI binary 入口
│   └── xhod.rs             # Daemon binary 入口
├── cli/
│   └── mod.rs               # CLI 逻辑（参数解析、交互模式、raw mode）
├── daemon/
│   ├── mod.rs               # daemon 启动、监听、shutdown、DaemonState
│   ├── rpc.rs               # XhoRpcService (execute, copy, status, list_servers)
...
│   │   ├── xhod.rs         # XhodGateway (SSH subsystem + gRPC client)
...
CLI
  ...
  → Daemon
      → for gw in gateways:
          ...
          ├─ XhodGateway: ensure_client → gRPC ListServers RPC → entries
          └─ JumpserverGateway: Err(Unsupported) → skip
      → 合并所有 entries
  → CLI 格式化显示
```

### 多候选 fallback `xho exec remote-asset-... -- ls`
...
CLI
  ...
  → Daemon
      → Resolver.resolve("remote-asset-...") →
          [Route { gateway:"remote-xhod", end_target:"remote-asset-..." },
           Route { gateway:"corp-jump", end_target:"remote-asset-..." }]
      → gateways["remote-xhod"].exec("remote-asset-...", req)
