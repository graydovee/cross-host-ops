# xho Setup & Deploy Reference

**One-time setup only:** deploying the remote **xhod** daemon, installing the
local **`xho` client**, and registering a gateway with token-based key
bootstrap. Read this when installing or adding a host — not for daily use.
Daily operations (`xho exec`/`cp`, transparent SSH proxy) live in SKILL.md.

## Table of Contents

- [Install / Deploy](#install--deploy)
- [Token-based bootstrap](#token-based-bootstrap)
...
(amd64 + arm64) and per-target tarballs. Use the bundled script (run from the
skill directory):
...
the proxy trusts *human* keys in `proxy_authorized_keys`. See
[config-and-usage.md](config-and-usage.md) for the full config format.
