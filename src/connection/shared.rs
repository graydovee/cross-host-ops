...
mod tests {
    ...
    use super::{
        build_final_command, build_remote_command, chmod_octal,
        download_destination_for_directory, extract_sentinel, is_tilde_user_path,
        join_remote_path, maybe_local_download_target, remote_path_needs_expansion,
        resolve_shell, shell_flags, shell_quote, split_tilde_path,
        upload_destination_for_directory, wrap_in_shell,
    };
    ...
    fn build_final_command_with_shell() {
        ...
        // Command name "ls" is unquoted for alias expansion, args are quoted
        // inner = "ls '-la'", wrapped = bash -ic 'ls '\''-la'\'''
        let expected = r#"bash -ic 'ls '\''-la'\'''"#;
        assert_eq!(result, expected);

        // Simple single-arg case: no quoting needed
