...
- 2026-08-27 [bug] uploading to a directory destination over the jumpserver path hung forever and never landed the file: the shell-copy upload targeted the directory itself, so its `.xho_tmp` partial was unwritable (`/tmp.xho_tmp`) and the dead receiver left the client writing into a stalled pipe; destinations now resolve scp-style (a directory root lands the file inside under the source name, matching the sftp path) in both the upload and the resume probe, and the raw-mode upload write phase gained a 60s write timeout plus early end-marker detection so a wedged relay or a dead receiver fails fast (leaving the partial for `--resume`) instead of freezing the progress bar
