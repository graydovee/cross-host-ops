...
- 2026-09-14 [feat] Windows 发布打包：tag CI 现在会构建 `x86_64-pc-windows-msvc` zip 包（与各平台 tarball 一起发布），Git Bash 下 `deploy.sh --local` 自动识别 Windows、下载 zip 并安装 `xho.exe`/`xdhod.exe` 到 `~/.bin`

## v0.5.8

- 2026-09-01 [bug] 修复 2222 透明代理在持续交互输出（vim、top）下永久冻结的问题；会话不再卡死，网关死链快速失败而非无限挂起
